...
No candidate is retained or promoted here. These families are independent of
rejected E1 (register-name clone avoidance), E2 (DFS join-write closure cache),
and E3 (dense dominance rows).
...
## Trial results and disposition

All three ranked families were tested once from parent `49787cc...`. Every
candidate was rejected, fully rolled back, and produced no product commit.

| Candidate | Measured result | Disposition |
| --- | --- | --- |
| Parent-linked block-ledger paths | All 660 resource cells were non-increasing. Combined allocated bytes fell from 22,304,232,271 to 13,136,079,343, including 60.75 percent for `fan-in/1024/base` and 59.53 percent for `multi-exit/1024/base`. The 15-pair per-pass combined improvement was only 1.627 percent against a 5 percent MDE. | Rejected and fully rolled back. |
| Single-pass quality counters | Serialization allocations fell from 15,600,734 to 171,047 and bytes from 188,593,267 to 109,918,819, with peak-live unchanged. Process RSS rose in all 165 rows, so the strict no-rise resource guard failed before timing. | Rejected and fully rolled back. |
| Batched naming | All 660 resource cells were non-increasing. Combined allocations fell from 336,200,562 to 321,108,210 and bytes from 22,304,232,271 to 22,091,354,028. Pooled emission time improved 9.61 percent, but the 15-pair per-pass combined improvement was only 2.862 percent against a 5 percent MDE. | Rejected and fully rolled back. |

The naming trial did not reach candidate-specific public real-binary
compatibility generation or replay. That later gate was not required after the
timing rejection. A replay of an existing 17,402-artifact tree was recorded
during the attempt, but that tree was not generated by the disposable candidate
binary. It is not candidate evidence and must not be described as a passed
candidate-specific compatibility check.
