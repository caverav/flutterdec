...
| `VAL-COMPAT-001` | A public pinned APK reproduces reference and current artifacts with adjudicated differences and stable current output. | Passed with three documented guard-polarity sites still open for semantic adjudication | [`compatibility baseline`](compat-baseline-real-binary.md), [`compatibility evidence`](compat-evidence/) |
