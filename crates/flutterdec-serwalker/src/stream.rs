use crate::constants::{UNSIGNED_END_OF_DATA_BYTE, SIGNED_END_OF_DATA_BYTE, UNSIGNED_MAX_DATA_PER_BYTE, DATA_BITS_PER_BYTE};
pub struct Stream<'a>
{
    byte_stream: &'a [u8],
...
impl<'a> Stream<'a>
{
    ...
    {
        if self.byte_stream.len() > pos && pos > 0 { self.curr_stream_offset = pos; }
    }
    ...
    pub fn read_modified_leb128(&mut self, sign_marker: u8) -> u64 // 8 bytes should be enough for anything...
    {
        ...
        if first_byte > UNSIGNED_MAX_DATA_PER_BYTE // if the first byte has its MSb set
        {
            self.advance_pos(1);
        ...
        loop {
            ...
            if byte & UNSIGNED_END_OF_DATA_BYTE == UNSIGNED_END_OF_DATA_BYTE { break; } // final byte
    ...
    pub fn read_u64(&mut self) -> u64
    {
        ...
        let num_slice = &self.byte_stream[self.curr_stream_offset..self.curr_stream_offset + u64_size];

        let converted_slice: [u8; 8] = num_slice
                              .try_into()
                              .expect("Slice wasn't 8 bytes long...");
    ...
    pub fn read_u32(&mut self) -> u32
    {
        ...
        let num_slice = &self.byte_stream[self.curr_stream_offset..self.curr_stream_offset + u32_size];

        let converted_slice: [u8; 4] = num_slice
                              .try_into()
                              .expect("Slice wasn't 4 bytes long...");
    ...
    {
        let first_nullbyte_pos = self.byte_stream
        .iter()
        .position(|&b| b == 0x00)
