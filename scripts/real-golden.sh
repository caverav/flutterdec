...
extract_report_metrics() {
  ...
  jq '{
    android_startup: {
      ...
      manifest_anchor_path_count: (.android_startup.bootstrap_chain.paths | map(select(.anchor_kind | startswith("manifest_"))) | length),
      launcher_anchor_path_count: (.android_startup.bootstrap_chain.paths | map(select(.anchor_kind == "manifest_launcher_activity")) | length),
      deeplink_anchor_path_count: (.android_startup.bootstrap_chain.paths | map(select(.anchor_kind == "manifest_deeplink_activity")) | length),
      application_anchor_path_count: (.android_startup.bootstrap_chain.paths | map(select(.anchor_kind == "manifest_application")) | length)
    },
    bootflow: {
