...
# baseline that reports build layout as a noise floor, or between an inert
# candidate and an A/B run that reports noise as a product effect, so it is
# checked directly instead of only through a 6 minute pipeline run. Both
# directions of the rule get a passing case and a failing case.
set -euo pipefail
...
# Equal product refs: equal binaries required.
...
# Different product refs: different binaries required.
check "A/B with different binaries is allowed"          0 "$aa" "$bb" "$d1" "$d2"
check "A/B with reversed differing digests is allowed"  0 "$aa" "$bb" "$d2" "$d1"
check "A/B with identical binaries aborts"              1 "$aa" "$bb" "$d1" "$d1"
check "A/B reversed refs with identical binaries aborts" 1 "$bb" "$aa" "$d2" "$d2"
