...
- when no explicit `--app-package` is provided, capped prioritization derives a package hint from the parsed manifest package (for example `oss.krtirtho.spotube` -> `spotube`) and boosts matching `package:<name>/...` functions so selected output stays focused on app-owned logic
