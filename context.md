...
- capped-function disassembly ordering now prioritizes likely high-value targets (entrypoint-like names, lifecycle/router selectors such as `createState`/`build`/`onGenerateRoute`, and deeplink/activity signals including object-pool `target_va` hints), applies app-package frequency boosts plus shallow entrypoint-callee frontier boosts, adds structural tie-break signals (function size and internal call out-degree), penalizes repeated non-generic names, and downranks `no isolate` markers plus `dart:isolate*` library paths so capped runs bias toward broader app-logic coverage instead of duplicated mapper/bootstrap/runtime helper families
